...
class Module:
    ...
    def formula_retrieval(collection_tensor, formula_index, query_vector):
        ...
        dist = F.pairwise_distance(collection_tensor,query_vec,p=2.0, eps=1e-6, keepdim=False) 
        index_sorted = torch.sort(dist, descending=False)[1]
        top_1000 = index_sorted[:1000]
        top_1000 = top_1000.data.cpu().numpy()
        euclidean_values = torch.sort(dist, descending=False)[0][:1000].data.cpu().numpy()
        result = {}
        count = 1
        for x in top_1000:
            doc_id = formula_index[x]
            score = euclidean_values[count - 1]
            result[doc_id] = score
            count += 1
        return result
        """
        ...
        for encoded_tuple in lst_encoded_tuples:
            ...
            try:
                if first:
                    ...
                    temp_vector = temp_vector + self.model.get_vector_representation(encoded_tuple) # + n-demension vector
                counter = counter + 1
