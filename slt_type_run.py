...
def main():
    map_file_path = "./Embedding_Preprocessing/slt_type_encoder.tsv" 
    config_file_path = "./Configuration/config/config_3"
    ...
    embedding_type = TupleTokenizationMode(2) # Type 
    tokenize_all = False # Tokenize all elements (such as numbers and text)
    tokenize_number = False # Tokenize the numbers or not
    is_wiki = True # The dataset is wiki or not
    read_slt = True # True: SLT, False: OPT
    ...
    if train_model:
        ...
        if do_retrieval:
            ...
            system.create_result_file(retrieval_result, "./Retrieval_Results/slt_type_res.tsv", "3")
    else:
